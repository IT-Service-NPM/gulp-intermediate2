import { describe, expect, it, beforeEach, beforeAll, afterAll, vitest }
  from 'vitest';
...
describe('intermediate2', () => {
  ...
  it('must be support streaming files processing', async () => {
    ...
    const testDestFiles = (await fs.promises.readdir(
      testDestFilesPath, { recursive: true }))
      .filter((testPath: string) => fs.statSync(
        path.join(testDestFilesPath, testPath)).isFile());
    ...
    expect(pluginDirsProcessor.mock.calls.length).toEqual(1);
    ...
    expect(fs.existsSync(tempDirPath),
      'temp dir must be deleted').toBeFalsy();
  });
});
